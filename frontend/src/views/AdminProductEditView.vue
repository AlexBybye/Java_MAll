<!-- frontend/src/views/AdminProductEditView.vue -->
...
async function loadProductData() {
    try {
        ...
        if (response.data.success) {
            ...
            productForm.stock = product.stockQuantity || product.stock; // 兼容两种字段名
            productForm.description = product.description || '';
            productForm.imageUrl = product.imageUrl || ''; // 加载图片URL
        }
    } catch (error: any) {
        errorMessage.value = error.response?.data?.message || '加载商品数据失败';
...
async function submitForm() {
    try {
        ...
        if (response.data.success) {
            ...
            errorMessage.value = response.data.message || '商品更新失败';
        }
    } catch (error: any) {
...
<template>
    <div class="admin-product-edit">
        ...
        <div class="product-form-container">
            ...
            <div v-if="isLoadingProduct" class="loading">加载商品数据中...</div>
